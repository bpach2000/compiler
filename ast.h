...
typedef struct ast_node {
  ...
  // Name for functions, variable declarations, expressions 
  // (name in the symtbl)
  char *name; 

  // Variables, function calls (can use for local symbol table for functions)
  SymTable *st_ref; 

  // Function num of args
  // Expressions - integer constants (only use for intCONST what the actual value is)
  int num; 

  // functions: child0 - stmt body, child1 params
  // statements: child0 - stmt itself, child1 the following statement
  // expressions: child0 - expression itself, child1 the following expression
  // operation operands - child0 operand1, child1 operand2
  // if - child0 expression, child1 then, child2 else
  // assg - child0 lhs, child1 rhs
  // while - child0 condition, child1 body
  // return - child0 expression to return 
  struct ast_node *child0, *child1, *child2;

  // a list of intermediate code instructions for executing that node and its children
  // list link of instructions 
  Instr *code_hd;
  Instr *code_tl;

  // The location where the expression’s value is stored at runtime
  // • the place field refers to a symbol table entry for a variable or temporary
